...
const getAccessToken = async (app, userId) => {
  try {
    ...
    const { zuid, accessToken, refreshToken, accessTokenExpires } = response[0][
    ...
    if (checkExpire(parseInt(accessTokenExpires), Date.now())) {
      ...
      updateAccessToken(app, zuid, access_token, expires_in);
      return access_token;
    }
  } catch (error) {
    console.log(error);
...
const updateAccessToken = async (app, userId, accessToken, expires_in) => {
  try {
    const zcql = app.zcql();
    const newAccessTokenExpires = Date.now() + parseInt(expires_in) * 1000;
    const query = `UPDATE ${config.oauthTableName} SET accessToken = ${accessToken}, accessTokenExpires = ${newAccessTokenExpires} WHERE zuid=${userId}`;
    const response = await zcql.executeZCQLQuery(query);
